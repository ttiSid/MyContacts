...
class CategoriesRepository {
  async findAll(orderBy = 'ASC') {
    ...
    const rows = await db.query(`SELECT * FROM categories ORDER BY name ${direction}`); // Nesse caso, não há problema em deixar a interpolação no envio da query, pois caso o valor passado seja diferente de 'DESC', se tornará 'ASC';
